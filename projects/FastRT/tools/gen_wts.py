...
def get_parser():
    ...
    parser.add_argument("--wts_path", default="./trt_demo", help="path to save tensorrt weights file(.wts)")
    parser.add_argument("--show_model", action="store_true", help="print model architecture")
    parser.add_argument("--verify", action="store_true", help="print model output for verify")
    parser.add_argument("--benchmark", action="store_true", help="preprocessing + inference time")
    parser.add_argument(
        "opts",
...
if __name__ == "__main__":
    ...
    if args.verify:
        input = torch.ones(1, 3, cfg.INPUT.SIZE_TEST[0], cfg.INPUT.SIZE_TEST[1]).to(cfg.MODEL.DEVICE) * 255.0
    ...
    if args.benchmark:
        ...
        print("--- %s seconds ---" % ((time.time() - start_time) / 100.0))
