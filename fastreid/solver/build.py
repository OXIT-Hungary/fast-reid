...
def maybe_add_freeze_layer(cfg: CfgNode, optimizer: Type[torch.optim.Optimizer]) -> Type[torch.optim.Optimizer]:
...
def build_optimizer(cfg, model, contiguous=True):
    ...
    if solver_opt == "SGD":
        return (
            maybe_add_freeze_layer(cfg, maybe_add_gradient_clipping(cfg, torch.optim.SGD))(
        ...
        return (
            maybe_add_freeze_layer(cfg, maybe_add_gradient_clipping(cfg, getattr(torch.optim, solver_opt)))(
                params.contiguous() if contiguous else params
            ),
            params,
        )
