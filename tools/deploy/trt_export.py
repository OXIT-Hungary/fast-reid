...
from fastreid.utils.logger import PathManager, setup_logger
...
def get_parser():
    ...
    parser.add_argument("--output", default="outputs/trt_model", help="path to save converted trt model")
    parser.add_argument(
        "--mode",
        default="fp32",
    ...
    parser.add_argument("--width", default=128, type=int, help="input image width")
    parser.add_argument("--channel", default=3, type=int, help="input image channel")
    parser.add_argument("--calib-data", default="Market1501", help="int8 calibrator dataset name")
    parser.add_argument("--onnx-model", default="outputs/onnx_model/baseline.onnx", help="path to onnx model")
    return parser
...
def onnx2trt(
    ...
    assert mode in [
        ...
        "fp16",
        "int8",
    ], "mode should be in ['fp32', 'fp16', 'int8'], " "but got {}".format(mode)
